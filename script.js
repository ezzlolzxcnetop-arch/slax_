...
import { 
    getFirestore, collection, addDoc, setDoc, doc, query, onSnapshot, 
    serverTimestamp, getDocs, where, limit, deleteDoc, updateDoc, getDoc 
...
async function initAuth() {
    ...
    catch (e) { console.error("SLAX AUTH ERROR:", e); loginError.textContent = "КРИТИЧЕСКАЯ ОШИБКА: СБОЙ АВТОРИЗАЦИИ FIREBASE."; }
}
initAuth();
...
async function validateInput(name, password) {
    if (!currentUser) { loginError.textContent = "ОШИБКА: АВТОРИЗАЦИЯ FIREBASE ОЖИДАЕТСЯ."; return false; }

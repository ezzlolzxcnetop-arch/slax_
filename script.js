...
// --- КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Уникальный ID сессии для вкладки ---
...
async function validateInput(name, password) {
    ...
    if (!password || password.length < 6 || password.length > 16) {
        loginError.textContent = "ОШИБКА: ПАРОЛЬ ДОЛЖЕН БЫТЬ 6-16 СИМВОЛОВ.";
        return false;
    }
    return true;
}
...
async function attemptLogin() {
    ...
    try {
        ...
        if (userData.password === password) {
            ...
            // Загружаем плагины пользователя если есть
            if (userData.plugins) {
            ...
            loginError.textContent = "ОШИБКА: ПАРОЛЬ НЕВЕРЕН.";
            playTone(200, 0.3);
        }
...
function grantAccess() {
    ...
    // Инициализация систем после входа
            ...
            <div class="mb-8">
                ...
                <div class="p-4 border-2 border-purple-800 rounded-lg">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="plugin-css" class="w-full h-40 bg-black border border-purple-700 text-purple-300 p-2 rounded font-mono text-sm" placeholder="/* Ваш CSS код здесь */
body { 
...
window.testPlugin = function() {
    ...
    if (!css.trim()) {
        addSystemMessage("ВВЕДИТЕ CSS КОД", true);
        return;
    }
    ...
    addSystemMessage("ПЛАГИН ПРОТЕСТИРОВАН (будет удален через 10 секунд)", false);
...
window.saveCustomPlugin = async function() {
    ...
    if (!name || !css) {
        addSystemMessage("ЗАПОЛНИТЕ ВСЕ ПОЛЯ", true);
        return;
    }
    ...
    voiceRecordingPanel.innerHTML = `
        ...
        <div class="flex items-center justify-center mb-4">
            <div id="voice-visualizer" class="flex items-end h-16 space-x-1">
                <!-- Барсы визуализатора будут добавлены динамически -->
            </div>
        </div>
...
async function startVoiceRecording() {
    if (!userSettings.voiceMessagesEnabled) {
        addSystemMessage("ГОЛОСОВЫЕ СООБЩЕНИЯ ОТКЛЮЧЕНЫ В НАСТРОЙКАХ", true);
        return;
    }
...
async function sendVoiceMessage() {
    if (audioChunks.length === 0) {
        addSystemMessage("НЕТ ЗАПИСАННЫХ ДАННЫХ", true);
        return;
    }
...
function initializeAdminSystem() {
    ...
    if (currentUsername === 'WINTER') {
        setTimeout(() => {
            ...
            addSystemMessage("АДМИН СИСТЕМА ИНИЦИАЛИЗИРОВАНА", false);
        }, 1000);
    }
}
...
async function initializeAdminDocuments() {
    try {
        console.log("Инициализация документов админа...");
        ...
        if (!adminEventsDoc.exists()) {
            console.log("Создаю документ admin_events/current");
            ...
            addSystemMessage("ДОКУМЕНТ СОБЫТИЙ СОЗДАН", false);
        } else {
        ...
        addSystemMessage(`ОШИБКА FIRESTORE: ${error.message}`, true);
    }
}

function subscribeToAdminEvents() {
    console.log("Подписываюсь на admin_events...");
    ...
    adminEventsUnsubscribe = onSnapshot(adminEventsRef, (docSnap) => {
        if (docSnap.exists()) {
            ...
            if (currentUsername === 'WINTER' && data.sessionId !== localSessionId) {
                if (wasRainbowActive !== isRainbowEventActive) {
                    addSystemMessage(`RAINBOW MODE: ${isRainbowEventActive ? 'АКТИВИРОВАН' : 'ВЫКЛЮЧЕН'}`, false);
                }
                if (wasSilentActive !== isSilentEventActive) {
                    addSystemMessage(`SILENT MODE: ${isSilentEventActive ? 'АКТИВИРОВАН' : 'ВЫКЛЮЧЕН'}`, false);
                }
                if (wasMusicActive !== isMusicEventActive) {
                    addSystemMessage(`BACKGROUND MUSIC: ${isMusicEventActive ? 'АКТИВИРОВАН' : 'ВЫКЛЮЧЕН'}`, false);
                }
            }
            ...
            if (statusEl) {
                statusEl.innerHTML = `<span class="text-green-400">СИНХРОНИЗИРОВАНО</span>`;
            }
            
        } else {
            console.warn("Документ событий не найден");
            if (currentUsername === 'WINTER') {
                initializeAdminDocuments();
            }
        }
        ...
        addSystemMessage("ОШИБКА СИНХРОНИЗАЦИИ СОБЫТИЙ", true);
    });
}
        ...
        snapshot.docChanges().forEach((change) => {
            if (change.type === 'added') {
                ...
                if (broadcast.text && broadcast.sender) {
                    addSystemMessage(`АДМИН [${broadcast.sender}]: ${broadcast.text}`, false);
                    ...
                    setTimeout(async () => {
                        try {
                            ...
                            console.log(`Админское сообщение ${broadcastId} удалено через 6 секунд`);
                        } catch (error) {
                            console.error(`Ошибка удаления сообщения ${broadcastId}:`, error);
                        }
                    }, 6000);
                }
            }
...
async function updateAdminEvent(eventName, value) {
    if (currentUsername !== 'WINTER') {
        addSystemMessage("ТРЕБУЮТСЯ ПРАВА АДМИНА", true);
        return;
    }
    
    try {
        console.log(`Обновление ${eventName} на ${value}`);
        ...
        const action = value ? "АКТИВИРОВАН" : "ВЫКЛЮЧЕН";
        addSystemMessage(`${eventNames[eventName]}: ${action}`, false);
        
        console.log(`${eventName} обновлен успешно`);
        
    } catch (error) {
        console.error(`Ошибка обновления ${eventName}:`, error);
    ...
    if (isActive) {
        ...
        backgroundMusic.play().then(() => {
            ...
            if (userSettings.notifications) {
                addSystemMessage("ФОНОВАЯ МУЗЫКА ВКЛЮЧЕНА", false);
            }
        }).catch(error => {
            console.error("Ошибка воспроизведения музыки:", error);
            if (error.name === 'NotAllowedError') {
                addSystemMessage("РАЗРЕШИТЕ АВТОВОСПРОИЗВЕДЕНИЕ МУЗЫКИ В БРАУЗЕРЕ", true);
            }
        });
    } else {
        ...
        if (userSettings.notifications) {
            addSystemMessage("ФОНОВАЯ МУЗЫКА ВЫКЛЮЧЕНА", false);
        }
    }
}
...
window.uploadMusic = async function() {
    ...
    if (!file) {
        addSystemMessage("ВЫБЕРИТЕ ФАЙЛ", true);
        return;
    }
    
    if (file.size > 1024 * 1024) {
        addSystemMessage("ФАЙЛ СЛИШКОМ БОЛЬШОЙ (МАКСИМУМ 1 МБ)", true);
        return;
    }
    
    if (!file.type.startsWith('audio/')) {
        addSystemMessage("ФАЙЛ ДОЛЖЕН БЫТЬ АУДИО", true);
        return;
    }
    ...
    reader.onloadend = async () => {
        ...
        try {
            ...
            addSystemMessage(`МУЗЫКА ЗАГРУЖЕНА: ${file.name}`, false);
            
            fileInput.value = '';
            
        } catch (error) {
            console.error("Ошибка загрузки музыки:", error);
            addSystemMessage(`ОШИБКА ЗАГРУЗКИ: ${error.message}`, true);
        }
    };
...
function updateAdminMusicInfo() {
    ...
    if (musicInfo) {
        musicInfo.textContent = currentMusicName ? `Загружен файл: ${currentMusicName}` : 'Музыка не загружена';
    }
}
...
window.sendAdminBroadcast = async function() {
    ...
    if (!broadcastText) {
        addSystemMessage("ВВЕДИТЕ ТЕКСТ СООБЩЕНИЯ", true);
        return;
    }
    ...
    try {
        ...
        setTimeout(async () => {
            try {
                ...
                console.log(`Админское сообщение ${broadcastDoc.id} удалено через 6 секунд`);
            } catch (error) {
                console.error(`Ошибка удаления сообщения ${broadcastDoc.id}:`, error);
            }
        }, 6000);
        ...
        addSystemMessage(`РАССЫЛКА ОТПРАВЛЕНА: "${broadcastText.substring(0, 50)}${broadcastText.length > 50 ? '...' : ''}"`, false);
        
        if (!isSilentEventActive) {
            playTone(1500, 0.1);
        }
        
        console.log("Рассылка отправлена:", broadcastText);
        
    } catch (error) {
        console.error("Ошибка отправки рассылки:", error);
        addSystemMessage(`ОШИБКА ОТПРАВКИ: ${error.message}`, true);
    }
};

window.testAdminSystem = function() {
    console.log("=== ТЕСТ АДМИН СИСТЕМЫ ===");
    ...
    addSystemMessage("ТЕСТ АДМИН СИСТЕМЫ ВЫПОЛНЕН (см. консоль)", false);
};
        ...
        <div class="admin-panel-container w-11/12 max-w-2xl max-h-[90vh] overflow-y-auto border-2 border-yellow-600 bg-black/95 p-6 shadow-[0_0_30px_rgba(255,255,0,0.5)]">
            <div class="flex justify-between items-center mb-4">
                ...
                <div class="flex space-x-2">
                    <button onclick="testAdminSystem()" class="px-3 py-1 border border-gray-600 text-xs hover:bg-gray-700" title="Тест системы">
                        ТЕСТ
                    </button>
            ...
            <div class="mb-6 p-3 bg-black/50 border border-yellow-800">
                <div class="text-sm text-yellow-300 mb-1">СТАТУС СИСТЕМЫ:</div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between p-3 border border-yellow-800 hover:border-yellow-600 transition-colors">
                        <div>
                            ...
                            <div class="text-xs text-yellow-600">Радужный режим терминала</div>
                        </div>
                        <div class="flex space-x-2">
                    ...
                    <div class="flex items-center justify-between p-3 border border-yellow-800 hover:border-yellow-600 transition-colors">
                        <div>
                            ...
                            <div class="text-xs text-yellow-600">Отключает все звуки сайта</div>
                        </div>
                        <div class="flex space-x-2">
                    ...
                    <div class="flex items-center justify-between p-3 border border-yellow-800 hover:border-yellow-600 transition-colors">
                        <div>
                            ...
                            <div class="text-xs text-yellow-600">Фоновая музыка на сайте</div>
                        </div>
                        <div class="flex space-x-2">
            ...
            <div class="mb-8">
                ...
                <div class="space-y-4">
                    <div class="p-3 border border-yellow-800 hover:border-yellow-600 transition-colors">
                        <div class="font-bold text-yellow-400 mb-2">ТЕКУЩАЯ МУЗЫКА</div>
                        <div id="current-music-info" class="text-xs text-yellow-600">
                            ${currentMusicName ? `Загружен файл: ${currentMusicName}` : 'Музыка не загружена'}
                        </div>
                    </div>
                    
                    <div class="p-3 border border-yellow-800 hover:border-yellow-600 transition-colors">
                        <div class="font-bold text-yellow-400 mb-2">ЗАГРУЗИТЬ НОВУЮ МУЗЫКУ</div>
                        <div class="text-xs text-yellow-600 mb-2">Выберите аудиофайл (MP3, до 1 МБ)</div>
            ...
            <div>
                ...
                <div class="mb-4">
                    <textarea id="admin-broadcast-text" 
                        ...
                        placeholder="Введите системное сообщение для всех пользователей..."></textarea>
                    <div class="text-xs text-yellow-600 mt-1">Сообщение появится у всех пользователей через 1-3 секунды</div>
                </div>
            ...
            <div class="text-xs text-yellow-700 text-center flex justify-between">
                <span>Пользователь: <span class="text-yellow-500">${currentUsername}</span></span>
                <span>Статус: <span id="admin-status" class="text-yellow-500">Ожидание синхронизации...</span></span>
                <span>Версия: <span class="text-yellow-500">1.3</span></span>
            </div>
        </div>
    `;
...
function updateAdminStatus() {
    ...
    if (rainbowStatus) {
        rainbowStatus.textContent = isRainbowEventActive ? 'АКТИВЕН' : 'ВЫКЛ';
        rainbowStatus.className = isRainbowEventActive ? 'text-green-400' : 'text-gray-400';
    }
    
    if (silentStatus) {
        silentStatus.textContent = isSilentEventActive ? 'АКТИВЕН' : 'ВЫКЛ';
        silentStatus.className = isSilentEventActive ? 'text-green-400' : 'text-gray-400';
    }
    
    if (musicStatus) {
        musicStatus.textContent = isMusicEventActive ? 'АКТИВЕН' : 'ВЫКЛ';
        musicStatus.className = isMusicEventActive ? 'text-green-400' : 'text-gray-400';
    }
    
    if (adminStatus) {
        if (currentUsername === 'WINTER') {
            adminStatus.innerHTML = `<span class="text-green-400">АДМИН | ${isRainbowEventActive ? 'RAINBOW' : ''}${isSilentEventActive ? ' SILENT' : ''}${isMusicEventActive ? ' MUSIC' : ''}</span>`;
        } else {
            adminStatus.innerHTML = `<span class="text-yellow-400">ТОЛЬКО ПРОСМОТР</span>`;
        }
    }
}
...
// --- ЧАСТЬ 5: ТЕКСТОВЫЙ ЧАТ И ГОЛОСОВЫЕ СООБЩЕНИЯ ---
...
function playVoiceMessage(base64Audio, duration, messageId) {
    ...
    if (messageElement) {
        ...
        if (playButton) {
            playButton.innerHTML = '<span class="text-white text-sm">⏸</span>';
    ...
    audio.play().catch(e => {
        ...
        addSystemMessage("ОШИБКА ВОСПРОИЗВЕДЕНИЯ АУДИО", true);
    });
}
...
function addSystemMessage(text, isError = false) {
    ...
    const isAdminEventNotification = 
        ...
        text.includes("ФОНОВАЯ МУЗЫКА ВКЛЮЧЕНА") ||
        text.includes("ФОНОВАЯ МУЗЫКА ВЫКЛЮЧЕНА");
...
// --- ЧАСТЬ 6: WEB-RTC / ГОЛОСОВОЙ ЧАТ И ЭКРАН ---
        ...
        snapshot.forEach(doc => {
            ...
            if (userData.sessionId !== localSessionId) {
                ...
                if (!pc || pc.connectionState === 'closed' || pc.connectionState === 'failed') {
                    ...
                    addSystemMessage(`ПОЛЬЗОВАТЕЛЬ ${userData.username} ПРИСОЕДИНИЛСЯ. (Initiator: ${isInitiator ? 'Yes' : 'No'})`);
                }
            }
        });
    ...
    signalUnsubscribes['local'] = onSnapshot(q, (snapshot) => {
        snapshot.docChanges().forEach(async (change) => {
            ...
            try {
                ...
                console.error("Ошибка удаления обработанного сигнала (возможно, уже удален):", e);
            }
        });
...
async function createPeerConnection(targetSessionId, isInitiator) {
    ...
    pc.onnegotiationneeded = async () => {
        if (isInitiator) {
            try {
                addSystemMessage(`ИНИЦИАЦИЯ ПЕРЕСОГЛАСОВАНИЯ...`);
                ...
                console.error("Ошибка создания/отправки OFFER в onnegotiationneeded:", e);
            }
        }
    };
    ...
    pc.oniceconnectionstatechange = () => {
        if (pc.iceConnectionState === 'disconnected' || pc.iceConnectionState === 'failed') {
            console.log(`ICE State for ${targetSessionId.slice(0, 4)}: ${pc.iceConnectionState}. Попытка восстановления...`);
        }
    };
    ...
    if (isInitiator) {
        try {
            ...
            console.error("Ошибка создания/отправки OFFER:", e);
        }
    }
}
    ...
    try {
        if (pc.signalingState !== 'stable') {
            if (pc.signalingState === 'have-local-offer') {
                ...
                console.warn("Получен OFFER в нестабильном состоянии, ожидание:", pc.signalingState);
                return;
            }
        }
        ...
        console.error("КРИТИЧЕСКАЯ ОШИБКА ОБРАБОТКИ OFFER:", e);
...
async function handleAnswer(senderSessionId, answer) {
    ...
    if (!pc) {
        console.warn(`Получен ANSWER, но PC для ${senderSessionId} не найден.`);
        return;
    }
    ...
    if (pc.signalingState === 'have-local-offer') {
        try {
            ...
            console.error("КРИТИЧЕСКАЯ ОШИБКА ОБРАБОТКИ ANSWER:", e);
            addSystemMessage(`ОШИБКА: СБОЙ ОБРАБОТКИ ОТВЕТА.`, true);
        }
    } else {
        console.warn(`Получен ANSWER в неожиданном состоянии: ${pc.signalingState}. Игнорируем.`);
    }
}
...
async function handleCandidate(senderSessionId, candidate) {
    ...
    if (pc && pc.remoteDescription) {
        try {
            ...
            console.error('Ошибка добавления ICE-кандидата (игнорируется):', e);
        }
    }
}
...
async function sendSignal(signalData, targetSessionId) {
    ...
    try {
        ...
        console.error(`Ошибка отправки сигнала к ${targetSessionId}:`, e);
    }
}
...
async function startScreenShare() {
    if (!currentVoiceChannel) {
        addSystemMessage("ОШИБКА: СНАЧАЛА ПОДКЛЮЧИТЕСЬ К ГОЛСОВОМУ КАНАЛУ.", true);
        return;
    }
